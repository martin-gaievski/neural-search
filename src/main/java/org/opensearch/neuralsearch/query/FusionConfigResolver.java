...
/**
 * Resolves the fused-mode fusion config from the request's attached search pipeline, reproducing core's pipeline-id
 * resolution precedence (see {@code SearchPipelineService.resolvePipeline}). This is the Option-P mechanism: a
 * fused-mode hybrid query reads the SAME normalization/combination config an existing hybrid user already has in their
 * pipeline, with no core change and zero migration.
 *
...
final class FusionConfigResolver {
    ...
    static FusionSpec resolve(SearchRequest searchRequest) {
        Map<String, Object> inlineConfig = searchRequest.source() == null ? null : searchRequest.source().searchPipelineSource();
        ...
        if (inlineConfig != null && namedPipeline != null) {
        ...
        if (indexDefaultId == null || NONE_PIPELINE_ID.equals(indexDefaultId)) {
            return null;
        }
