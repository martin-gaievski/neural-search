...
final class HybridFusionOrchestrator {
    ...
    static SearchHits assemblePage(RankedDocs ranked, SearchSourceBuilder source, TotalHits totalHits) {
        ...
        for (int i = from; i < end; i++) {
            ...
            // The legs sorted by [_score, _doc] to expose the doc id; round 2 without a sort reports no sort values.
            hit.sortValues(new Object[0], new DocValueFormat[0]);
            page[i - from] = hit;
        }
    ...
    /**
     * The ranked window in the order round 2 would have returned it. Round 2's Top scores each document at its fused
     * score, and core merges the shards' results with {@code TopDocs.merge}: score descending, then the shard's position
     * in the request's shard iteration (which is {@code ShardId} order), then the shard's own order — doc id ascending for
     * a score collector. The fused ranking already has the scores in order; this settles equal scores the same way,
     * reading each hit's shard from its target and its doc id from the {@code _doc} sort value the legs were asked for.
     * A hit without those (a leg that was not sorted) keeps its fused position, so this never reorders across scores.
     */
    private static Integer[] roundTwoOrder(RankedDocs ranked) {
        ...
        Arrays.sort(order, (a, b) -> {
            ...
            if (Objects.isNull(first.getShard()) || Objects.isNull(second.getShard())) {
            ...
            Integer firstDoc = lucenedocId(first);
            Integer secondDoc = lucenedocId(second);
            if (Objects.isNull(firstDoc) || Objects.isNull(secondDoc)) {
                return Integer.compare(a, b);
            }
            return Integer.compare(firstDoc, secondDoc);
        });
        return order;
    }

    /** The Lucene doc id a {@code [_score, _doc]}-sorted leg reports as the hit's second sort value, or null without it. */
    private static Integer lucenedocId(SearchHit hit) {
        Object[] sortValues = hit.getSortValues();
        if (Objects.isNull(sortValues) || sortValues.length < 2 || (sortValues[1] instanceof Number) == false) {
            return null;
        }
        return ((Number) sortValues[1]).intValue();
    }
     ...
     * Both paths report through the same consumers, so a caller cannot tell them apart except by latency and, when
     * stats are on, {@code hybrid_query_fused_fast_path_requests}.
     */
