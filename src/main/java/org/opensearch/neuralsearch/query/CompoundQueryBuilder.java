...
public class CompoundQueryBuilder extends AbstractQueryBuilder<CompoundQueryBuilder> {
    ...
    protected Query doToQuery(QueryShardContext queryShardContext) throws IOException {
        Collection<Query> luceneQueries = toQueries(queries, queryShardContext);
        if (luceneQueries.isEmpty()) {
            return Queries.newMatchNoDocsQuery("no clauses for compound query.");
        }

        return new CompoundQuery(luceneQueries);
    }
    ...
    static Collection<Query> toQueries(Collection<QueryBuilder> queryBuilders, QueryShardContext context) throws QueryShardException,
        ...
        for (QueryBuilder queryBuilder : queryBuilders) {
            PlainActionFuture<QueryBuilder> f = new PlainActionFuture<>();
            Rewriteable.rewriteAndFetch(queryBuilder, context, f);
            try {
                Query query = f.get().toQuery(context);
                if (query != null) {
                    queries.add(query instanceof KnnVectorQuery ? query.rewrite(context.getIndexReader()) : query);
                }
            } catch (Exception e) {
