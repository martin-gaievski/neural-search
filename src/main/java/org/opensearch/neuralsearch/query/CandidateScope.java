...
final class CandidateScope {
    ...
    SearchRequest newLegRequest(final QueryBuilder leg, final int windowSize) {
        ...
        if (Objects.isNull(legFetchSource)) {
            ...
            // The page will be assembled from these hits, so equal fused scores have to come out in the order round 2
            // would have produced: score, then shard, then Lucene doc id (TopDocs.merge's tie-break). A _score-first
            // sort keeps the collector's score-based skipping, core sets each hit's _score from the sort value, and the
            // trailing _doc sort value is the doc id the assembly orders ties by.
            legSource.sort(SortBuilders.scoreSort()).sort(SortBuilders.fieldSort(FieldSortBuilder.DOC_FIELD_NAME));
        }
