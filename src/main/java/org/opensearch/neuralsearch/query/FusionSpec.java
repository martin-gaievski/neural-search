...
/**
 * Immutable fusion configuration for the hybrid query {@code mode: "fused"} path: the normalization + combination
 * technique, per-leg weights, and RRF rank_constant. In fused mode the config is NOT parsed from the query body — it
 * is read from the attached search pipeline's phase-results processor at coordinator rewrite (same source as classic
 * hybrid), reproducing zero-migration UX. This class reads that config from a resolved pipeline config map.
 *
...
public final class FusionSpec {
    ...
    static final String NORMALIZATION_Z_SCORE = ZScoreNormalizationTechnique.TECHNIQUE_NAME;
    static final String NORMALIZATION_L2 = L2ScoreNormalizationTechnique.TECHNIQUE_NAME;
    ...
    FusionSpec(String combinationTechnique, String normalizationTechnique, int rankConstant, float[] weights) {
        ...
        this.normalizationTechnique = normalizationTechnique == null ? NORMALIZATION_NONE : normalizationTechnique;
