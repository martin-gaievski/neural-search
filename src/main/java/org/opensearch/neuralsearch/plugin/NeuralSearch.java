...
public class NeuralSearch extends Plugin
    ...
    public List<QuerySpec<?>> getQueries() {
        return Arrays.asList(
            ...
            // Internal query the hybrid fused mode self-erases into; registered so it serializes to data nodes.
            // fromXContent throws — it is never parsed from a request, only built at the coordinator and read on the wire.
    ...
    public Map<String, SystemGeneratedProcessor.SystemGeneratedFactory<SearchResponseProcessor>> getSystemGeneratedResponseProcessors(
        ...
        // a fused-mode hybrid profiler processor that merges per-sub-query (leg) profiles into the response profile, and a
        // fused-mode aggregations processor that swaps in the aggregation leg's true leg-union aggregations/total_hits.
        return Map.of(
            ...
            HybridFusedAggregationsResponseProcessor.TYPE,
            new HybridFusedAggregationsResponseProcessor.Factory()
        );
    }
