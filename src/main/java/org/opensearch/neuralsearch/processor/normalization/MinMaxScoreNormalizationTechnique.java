        ...
        for (Object boundObj : lowerBoundsParams) {
            ...
            try {
                LowerBound.Mode mode = LowerBound.Mode.fromString(
                    Objects.isNull(lowerBound.get("mode")) ? "" : lowerBound.get("mode").toString()
                );
                float minScore;
        ...
        protected enum Mode {
            APPLY {
                ...
                public float normalize(float score, float minScore, float maxScore, float lowerBoundScore) {
                    // if we apply the lower bound this mean we return actual score in case it's less then the lower bound min score
            ...
            CLIP {
                @Override
                public float normalize(float score, float minScore, float maxScore, float lowerBoundScore) {
                    // apply clipping, return lower bound min score if score is less than min score
                    if (maxScore < lowerBoundScore) {
                        return (score - minScore) / (maxScore - minScore);
                    }
                    if (score < minScore) {
            ...
            IGNORE {
                @Override
                public float normalize(float score, float minScore, float maxScore, float lowerBoundScore) {
                    // ignore lower bound logic and do raw min-max normalization
