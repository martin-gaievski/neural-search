...
* In-query fusion in hybrid search. Skip round 2's Tail when the legs already prove the request's `hits.total`: a fused hybrid that is the request's own query and wants a count beyond its window now has its lexical legs count up to the `track_total_hits` threshold, and when a leg exceeds it round 2 runs Top-only and the response carries the same `{threshold, gte}` the Tail would have reported. Requests whose Tail documents are part of the answer (aggregations, highlight, any sort other than `_score` descending, collapse, `search_after`, `min_score`, a page past the fused window, a named ANN leg that filled its window, exact totals) keep the Tail unchanged, as do requests whose search pipeline has response processors ([#TBD](https://github.com/opensearch-project/neural-search/pull/TBD))
