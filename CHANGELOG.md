...
* In-query fusion in hybrid search. Define `rescore` semantics in fused mode: a `rescore` may override the hybrid's score scale and reorder its hits, but can no longer promote a document the fusion never ranked — every `rescore` query in the chain is confined to the fused window, which closes the `rescore.window_size > fusion.window_size` case, the per-shard window shortfall and the same widening inside `top_hits` buckets; the confinement is installed before the leg fan-out so it survives a `rescore` query that rewrites (`wrapper`, `neural`, `neural_sparse`, `terms` lookup), and the request is failed rather than answered if it cannot be; a rescorer type other than `query` is rejected with a validation error in fused mode; fused scores are floored to a small positive value so a ranked document outranks the non-scoring tail even after `query_weight` or an enclosing `boost` attenuates it, and a non-finite or negative fused score is now rejected instead of silently floored ([#1973](https://github.com/opensearch-project/neural-search/pull/1973))
