...
* In-query fusion in hybrid search. Fix ceiling's counting scope (leg budget counted a fan-out of zero) when Core rewrites `post_filter`, aggregation filters, `rescore` and `highlight_query` ([#1969](https://github.com/opensearch-project/neural-search/pull/1969))
